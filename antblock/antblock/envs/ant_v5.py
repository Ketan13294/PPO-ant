...
from antblock.envs import MujocoEnv
...
class AntBlockEnv(MujocoEnv, utils.EzPickle):
    """
    ### Description
    ...
    The ant is a 3D rob ot consisting of one torso (free rotational body) with
    ...
    *(x-coordinate before action - x-coordinate after action)/dt*. *dt* is the time
    ...
    @property
    def done(self):
        done = self.is_healthy and self.model.body_pos[0][0]>self.model.geom_size[14][0]/2
        return done
    ...
    def _get_obs(self):
        ...
        position[0] -= self.model.geom_pos[14][0]
        ...
        if self._use_contact_forces:
            ...
            return np.concatenate((position, velocity,np.array([self.model.body_pos[14][0],self.model.body_pos[14][2]])))
